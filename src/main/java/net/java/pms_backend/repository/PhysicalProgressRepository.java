...
public interface PhysicalProgressRepository extends JpaRepository<PhysicalProgress, Long> {
    ...
    @Query("SELECT SUM(p.completedPercentage) FROM PhysicalProgress p " +
            "WHERE p.projectWorkParameter.id = :parameterId")
    BigDecimal getTotalLoggedByParameterId(@Param("parameterId") Long parameterId);

    @Query("SELECT SUM(p.completedPercentage) FROM PhysicalProgress p " +
            "WHERE p.projectWorkParameter.id = :parameterId AND p.id != :excludedId")
    BigDecimal getTotalLoggedByParameterIdExcludingId(@Param("parameterId") Long parameterId,
